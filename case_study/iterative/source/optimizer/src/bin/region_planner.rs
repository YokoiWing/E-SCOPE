...
use mac_egg::adaptive_region::{AdaptiveRegionSpace, RegionAssignment, build_rule_agnostic_space};
...
mod tests {
    ...
    fn phase_i_unresolved_domain_uses_existing_combinations() {
        let lib = Path::new("../netlist/test/asap7sc6t_SELECT_LVT_TT_nldm.lib");
        let pins = read_pin_map_for_test(lib);
        let (source, _) =
            read_verilog_with_lib_to_netlist("../netlist/test/topology_inv_and.v", pins).unwrap();
        let (space, _) = build_rule_agnostic_space(
            &source,
            &[Path::new("../netlist/test/6t_adaptive_dummy_rules.json")],
            2,
            3,
        )
        .unwrap();
