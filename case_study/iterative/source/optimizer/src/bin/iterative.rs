...
#[path = "evaluate_batch.rs"]
#[allow(dead_code)]
mod p1_impl;
#[path = "region_planner.rs"]
#[allow(dead_code)]
mod planner_impl;
#[path = "rerank.rs"]
#[allow(dead_code)]
...
fn main() -> Result<()> {
    ...
    for round_index in 0..max_rounds {
        ...
        rounds.push(receipt);
        let summary = json!({
            "method":if unified_equivalence_enabled() {
                "D1 ordinary enodes + Generator virtual enodes through unified topology extraction + native progressive refinement + fresh-parent loop"
            } else {
                "D1 rewrite + native Rust Generator V2 union + native progressive refinement + fresh-parent loop"
            },
