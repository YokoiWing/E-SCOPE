...
impl Extractor for PrioQueueExtractor {
    fn extract(&self, egraph: &ExtendedEGraph, _roots: &[ClassId]) -> ExtractionResult {
        ...
        let n2c = |nid: &NodeId| egraph.inner.nid_to_cid(nid);
        let mut analysis_pending: PrioQueue<NodeId, NotNan<f64>> = PrioQueue::new();
        ...
        for class in egraph.inner.classes().values() {
            for node in &class.nodes {
                let child_classes: FxHashSet<&ClassId> =
                    egraph.inner[node].children.iter().map(n2c).collect();
                ...
                if egraph.inner[node].is_leaf() {
                    ...
                    analysis_pending.insert(node.clone(), cost.abs());
                }
            }
        }
        ...
        while let Some((node_id, _cost)) = analysis_pending.pop() {
            ...
            let cost = result.node_sum_cost(egraph, &node_id, &costs);
            ...
            for p in parents[class_id].iter() {
                ...
                *ctr -= 1;
                if *ctr == 0 {
                    let cost = result.node_sum_cost(egraph, p, &costs);
                    analysis_pending.insert(p.clone(), cost.abs());
                }
            }
        }
