...
fn expand_technology_mappings(
    ...
    areas: &BTreeMap<String, f64>,
    delays: &BTreeMap<String, f64>,
    expand_drive_mappings: bool,
) -> Result<Vec<GeneratorCandidate>> {
    let mut variants = Vec::new();
    let mut seen = HashSet::new();
    let mapping_names: &[&str] = if expand_drive_mappings {
        &["area", "balanced", "timing"]
    } else {
        // Generator expressions are constructed with the area mapping. Keep
        // that native seed implementation without adding alternate drives of
        // the same logical structure during Phase I.
        &["area"]
    };
...
pub struct GeneratorConfig {
    ...
    /// Add balanced/timing drive realizations for each generated logical
    /// structure. The default preserves the frozen Iterative proposal stream.
    pub expand_drive_mappings: bool,
}
...
pub fn generate_deletion_candidates(
        ...
        .filter(|_| !bounded_functional_fast_root_screen)
    {
        ...
        for cone_depth in [2usize, 3] {
            ...
            for (term_index, term) in terms.into_iter().take(3).enumerate() {
                ...
                for candidate in expand_technology_mappings(
                    &base,
                    &mappings,
                    &areas,
                    &delays,
                    expand_drive_mappings,
                )? {
    ...
    if enable_mapped_recipes && !bounded_functional_fast_root_screen {
        ...
        for (root, cut_index, boundary, target) in mapped_cuts {
            ...
            for (recipe_index, recipe) in recipes.iter().enumerate() {
                ...
                for mut candidate in expand_technology_mappings(
                    &base,
                    &mappings,
                    &areas,
                    &delays,
        ...
        .filter(|_| !bounded_functional_fast_root_screen)
    {
        ...
        for (term_index, term) in terms.into_iter().take(3).enumerate() {
            ...
            for candidate in
                expand_technology_mappings(&base, mappings, areas, delays, expand_drive_mappings)?
            {
                let saving = incumbent_area - candidate.tech_area;
                if saving <= 1e-12 {
                    continue;
    ...
    fs::write(
        ...
        serde_json::to_string_pretty(&json!({
            ...
            "drive_mapping_expansion_enabled":expand_drive_mappings,
...
pub fn generate(
    ...
    fs::write(
        ...
        serde_json::to_string_pretty(&json!({
            ...
            "drive_mapping_expansion_enabled":config.expand_drive_mappings,
...
mod tests {
    ...
    #[test]
    fn phase_i_drive_mapping_switch_keeps_only_native_area_seed() {
        let area_name = "AND2x1_ASAP7_6t_L";
        let balanced_name = "AND2x2_ASAP7_6t_L";
        let timing_name = "AND2x4_ASAP7_6t_L";
        let tech_cell = |name: &str, area: f64, delay: f64| TechCell {
            name: name.to_owned(),
            area,
            delay,
        };
        let mappings = BTreeMap::from([
            (
                "area".to_owned(),
                BTreeMap::from([("AND2".to_owned(), tech_cell(area_name, 1.0, 3.0))]),
            ),
            (
                "balanced".to_owned(),
                BTreeMap::from([("AND2".to_owned(), tech_cell(balanced_name, 2.0, 2.0))]),
            ),
            (
                "timing".to_owned(),
                BTreeMap::from([("AND2".to_owned(), tech_cell(timing_name, 3.0, 1.0))]),
            ),
        ]);
        let areas = BTreeMap::from([
            (area_name.to_owned(), 1.0),
            (balanced_name.to_owned(), 2.0),
            (timing_name.to_owned(), 3.0),
        ]);
        let delays = BTreeMap::from([
            (area_name.to_owned(), 3.0),
            (balanced_name.to_owned(), 2.0),
            (timing_name.to_owned(), 1.0),
        ]);
        let window = Window {
            region_id: "test".to_owned(),
            region: BTreeSet::from([2]),
            kind: "test".to_owned(),
            host: 2,
        };
        let base = empty_candidate(
            ...
            vec![GeneratorChoice {
                root_anchor: 2,
                expression: GeneratorExpr::Cell {
                    op: area_name.to_owned(),
                    children: vec![
                        GeneratorExpr::Anchor { anchor: 0 },
                        GeneratorExpr::Anchor { anchor: 1 },
                    ],
                },
            }],
            vec![2],
        ...
        let full = expand_technology_mappings(&base, &mappings, &areas, &delays, true).unwrap();
        assert_eq!(
            full.iter()
                .map(|candidate| candidate.mapping.as_deref().unwrap())
                .collect::<Vec<_>>(),
            vec!["area", "balanced", "timing"]
        );

        let ablated = expand_technology_mappings(&base, &mappings, &areas, &delays, false).unwrap();
        assert_eq!(ablated.len(), 1);
        assert_eq!(ablated[0].mapping.as_deref(), Some("area"));
        assert_eq!(ablated[0].candidate_id, "SEED_AREA");
        assert!(matches!(
            &ablated[0].choices[0].expression,
            GeneratorExpr::Cell { op, .. } if op == area_name
        ));
        assert!(GeneratorConfig::default().expand_drive_mappings);
    }
