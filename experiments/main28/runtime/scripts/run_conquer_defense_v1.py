...
from conquer_defense_policy_v1 import select_sources, endpoint_sets, pair_moves, closure_moves, make_plan, finalists
...
from run_iterative_da_sweep_v1 import frozen_env, g0_path, ANCHORS, LIB, SCALE, RULES
...
def main():
    ...
    policy_receipt = {"configuration": cfg, "config_sha256": sha(args.config),
        "script_sha256": sha(Path(__file__)), "policy_sha256": sha(D1 / "scripts/conquer_defense_policy_v1.py"),
        "frozen_unix": started, "historical_winner_inputs": [], "external_feedback_to_search": False}
    if promotion_v2:
        policy_receipt["promotion_policy_sha256"] = sha(D1 / "scripts/conquer_defense_policy_v2.py")
    if args.resume:
