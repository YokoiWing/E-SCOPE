...
from closure_policy import endpoint_sets, endpoint_sets_iterative, pair_moves, closure_moves, make_plan
...
from normal_lane import available_memory_kib, normal_policy
from runtime_utils import dump, load, sha, process_tree_rss
from search_environment import AUDIT, frozen_env, g0_path, ANCHORS, LIB, SCALE, RULES
...
def main():
    ...
    input_paths = [
        ...
        D1 / "scripts/closure_policy.py",
        D1 / "scripts/finalist_policy.py",
        D1 / "scripts/recall_policy.py",
        Path(__file__),
        D1 / "scripts" / (
    ...
    policy_receipt = {
        ...
        "recall_policy_sha256": sha(D1 / "scripts/recall_policy.py"),
        "defense_policy_sha256": sha(D1 / "scripts/finalist_policy.py"),
        "frozen_unix": started,
    ...
    for benchmark in args.cases.split(","):
        ...
        if cfg.get("formal_against_verified_g0", False):
            from validate_reference import checked_reference, CHECKS
